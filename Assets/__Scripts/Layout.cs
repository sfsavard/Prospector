...
public class Layout : MonoBehaviour
{
    ...
    public void ReadLayout(string xmlText)
    {
        ...
        for (int i = 0; i < slotsX.Count; i++)
        {
            ...
            tSD.layerName = sortingLayerNames[tSD.layerID];              // a The layerName field of SlotDef is used to make sure that the correct cards are on top of the others.
            //In Unity 2D, all of your assets are effectively at the same Z depth, so the layer is used to differentiate between them and determine which appears on top
...
//The SlotDef class is created to store information read in from the XML <slot>s in a more accessible way.
//Then, the Layout class is defined, and the ReadLayout() method is created, which will take an XML-formatted string as input and turn it into a series of SlotDefs
